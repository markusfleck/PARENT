...
double get_mutual(int type,int index1, int index2,int nDihedrals, double* bondsEntropy1D, double* anglesEntropy1D, double* dihedralsEntropy1D, double* bbEntropy, double* baEntropy, double* bdEntropy, double* aaEntropy, double* adEntropy, double* ddEntropy, int suppressWarning) {
    ...
    if((type==TYPE_DD)&&(index1!=index2)) { // for mutual information between two different dihedrals
        smaller=index1<index2?index1:index2;
        bigger=index1<index2?index2:index1;
    ...
    if(suppressWarning==0){cerr<<"WARNING: REQUEST FOR MUTUAL INFORMATION TYPE "<<type<<" WITH INDICES "<<index1<<" AND "<<index2<<"YIELDED NO RESULT."<<endl;}
    return 0;
}
